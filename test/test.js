...
describe('Schema test', function() {
  ...
  describe('Schema Properties', function() {
    it('Schema can has multiple validations for row', function() {
      const schema = {
        'RA_Report #': [
          { type: VALIDATIONS_ENUM.MIN, value: 65324, message: 'Invalid min value' },
          { type: VALIDATIONS_ENUM.TYPE, value: 'string', message: 'Invalid type' }
        ]
      };
    ...
    it('Schema can validate multiple columns', function() {
      const schema = {
        'RA_Report #': [{ type: VALIDATIONS_ENUM.MIN, value: 65324, message: 'Invalid min value' }],
        'CI_Age Unit': [{ type: VALIDATIONS_ENUM.MIN_LENGTH, value: 8, message: 'Invalid min lenght' }]
      };
      assert.equal(validation.isValid({ row, schema }), false);
    });
